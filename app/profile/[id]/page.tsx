...
export default function PublicProfilePage() {
  ...
  if (notFound || !profile) {
    return (
      <div style={{ minHeight: '100dvh', backgroundColor: '#FAFAFA', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 32px', textAlign: 'center' }}>
        <p style={{ fontSize: 16, fontWeight: 800, color: '#0a0a0a', marginBottom: 8 }}>Profile not found</p>
        <p style={{ fontSize: 14, color: '#AAAAAA', marginBottom: 28 }}>This seller profile doesn't exist or has been removed.</p>
        <button
          onClick={() => router.back()}
          style={{ fontSize: 14, fontWeight: 700, color: '#1D7A47', background: 'none', border: 'none', cursor: 'pointer' }}
        >
          Go back
        </button>
      </div>
    ...
    <>
      <style>{`
        ...
        * { font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; }
        ...
        .back-btn:active { transform: scale(0.93); opacity: 0.75; }
      `}</style>
      ...
      <div style={{ backgroundColor: '#F7F8F7', minHeight: '100dvh' }}>
        ...
        <div style={{
          ...
          {/* Back button row */}
          <div style={{ display: 'flex', justifyContent: 'flex-start', padding: '0 20px', marginBottom: 20 }}>
            <button
              onClick={() => router.back()}
              className="back-btn"
              aria-label="Go back"
        ...
        <div style={{ padding: '16px 14px 96px' }}>
          ...
          {/* Store tab — active listings only */}
          {tab === 'store' && (
            activeListings.length === 0 ? (
              <EmptyState
                icon={<EmptyJerseyIcon />}
                title="No active listings"
                subtitle="This seller doesn't have any jerseys for sale right now."
              />
            ) : (
              <>
                <p style={{
          ...
          {tab === 'reviews' && (
            reviews.length === 0 ? (
              <EmptyState
                ...
                subtitle="This seller hasn't received any reviews from buyers yet."
              />
            ) : (
              <>
                {avgRating && (
...
function EmptyState({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '56px 24px', textAlign: 'center' }}>
      ...
      <p style={{ fontSize: 14, color: '#AAAAAA', lineHeight: 1.55, maxWidth: 250 }}>{subtitle}</p>
    </div>
  )
}
...
function BackIcon() {
  return (
    <svg width="11" height="11" viewBox="0 0 24 24" fill="none">
      <path d="M19 12H5M12 19l-7-7 7-7" stroke="#555555" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
